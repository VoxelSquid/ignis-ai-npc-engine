package me.voxelsquid.ignis.quest.base

import me.voxelsquid.ignis.Ignis.Companion.gson
import me.voxelsquid.ignis.Ignis.Companion.pluginInstance
import me.voxelsquid.ignis.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.addQuest
import me.voxelsquid.ignis.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.gender
import me.voxelsquid.ignis.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.getCharacterType
import me.voxelsquid.ignis.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.professionLevelName
import me.voxelsquid.ignis.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.settlement
import me.voxelsquid.ignis.gameplay.humanoid.HumanoidManager.PersonalityType
...
import me.voxelsquid.ignis.gameplay.util.InventorySerializer.Companion.toBase64
...
import me.voxelsquid.ignis.runtime.DatabaseManager.QuestItem.Companion
...
open class Quest(val villager: Villager, private val questType: Type, val requiredItem: ItemStack = ItemStack(Material.AIR), private val gatheringQuestType: GatheringQuestType = GatheringQuestType.NONE, val requiredQuestItem: DatabaseManager.QuestItem = DatabaseManager.QuestItem.EMPTY, private val targetEntityType: String = EntityType.UNKNOWN.toString(), private val score: Int = requiredItem.calculatePrice(), private val quantity: Int = 1, var questData: QuestData = QuestData(questType, System.currentTimeMillis(), score, toBase64(requiredItem), gatheringQuestType = gatheringQuestType, targetEntityType = targetEntityType, questItem = requiredQuestItem, quantity = quantity)) {
    ...
    val placeholders = mutableMapOf(
        ...
        "villagerRace"            to (villager.race?.name ?: "human"),
        ...
        "extraArguments"          to if (plugin.controller.swearing) when (villager.getCharacterType()) {
    ...
    fun generate() {
        plugin.geminiProvider.sendGenerationRequest(this.prepareBasicGenerationPrompt()) { cleanedQuestJson ->
            val generatedData = gson.fromJson(cleanedQuestJson, QuestData::class.java)
    ...
    companion object {
        ...
        val questCountKey = NamespacedKey(pluginInstance, "TotalQuestCount")
        ...
        fun calculateBounty(villager: Villager, score: Int) : ItemStack {
            ...
            var amount = score / race.normalCurrency.getMaterialPrice()

            if (amount < 64) return ItemStack(race.normalCurrency, amount)

            val items = mutableListOf<ItemStack>()
            while (amount > 64) {
                items.add(ItemStack(race.normalCurrency, 64)); amount -= 64;
            }
            items.add(ItemStack(race.normalCurrency, amount))
