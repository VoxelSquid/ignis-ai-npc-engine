...
import me.voxelsquid.ignis.Ignis.Companion.dialogueFormat
...
class DialogueManager(private val plugin: Ignis) {
    ...
    companion object {
        ...
        fun Villager.talk(player: Player, text: String?, displaySize: Float = dialogueBoxSize, followDuringDialogue: Boolean = true, interruptPreviousDialogue: Boolean = false, onFinish: () -> Unit = {}) {
            text?.let { pluginInstance.humanoidManager.dialogueManager.startDialogue(player to this, it, size = displaySize, follow = followDuringDialogue, interrupt = interruptPreviousDialogue, onFinish = onFinish) }
        }
